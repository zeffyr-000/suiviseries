...
**Theme**: "Nuit & Or" — single dark, cinematic theme (deep night-blue surfaces + gold accent).
**Last Updated**: May 29, 2026
**M3 Conformity Score**: 98/100 ⭐

> 🕰️ For the app's visual evolution (V1 light → V2 dark), see
...
Two typefaces, both loaded locally via `@fontsource` (no external CDN):

- **Poppins** (`brand-family`) — display typeface for `.display-*` and `.headline-*`
  titles, giving a distinctive cinematic identity (weights 400/500/600).
- **Roboto** (`plain-family`) — body, labels and UI text (weights 300/400/500/700).

Wired in `styles.scss` via the `mat.theme((typography: (brand-family: 'Poppins',
plain-family: 'Roboto')))` block.
...
### Theme: "Nuit & Or" (single dark theme)

The app ships a **single dark theme** (no light/dark toggle). It is driven entirely by
M3 tokens: the palette is generated by `ng generate @angular/material:theme-color`
into `src/styles/_theme-colors.scss`, and `html { color-scheme: dark; }` resolves all
`--mat-sys-*` tokens to their dark values (deep night-blue surfaces, e.g. `#0e1227`).

Changing the entire identity is centralized in `src/styles.scss` (the `mat.theme(...)`
block + the custom CSS variables below).
...
> primary `#C8A951` (gold), secondary/tertiary night-blue, neutral `#2A2E45` (night-blue tinted).
