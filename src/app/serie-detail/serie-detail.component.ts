...
export class SerieDetailComponent implements OnInit {
    ...
    private buildSerieStats(
        serie: Serie,
        stats: { seasons_count: number; episodes_count: number } | undefined,
        isReallyFollowed: boolean,
    ): SerieStats {
        return {
        ...
        this.route.params
            .pipe(
                ...
                switchMap((response) => {
                    if (response?.success) {
                        ...
                        return this.seriesService.isSerieReallyFollowed(serie.id).pipe(
                            map((isReallyFollowed) => ({ serie, stats, isReallyFollowed })),
                            catchError(() =>
                                of({
                                    serie,
                                    stats,
                                    isReallyFollowed: serie.user_data?.is_following ?? false,
                                }),
                            ),
                        );
                    } else {
                        throw new Error(
                            this.transloco.translate('notifications.errors.serie_not_found'),
                        );
                    }
                }),
            ...
            .subscribe({
                ...
                error: (err) => {
                    this.error.set(
                        err?.message ||
                            this.transloco.translate('notifications.errors.loading_serie'),
                    );
                    this.loading.set(false);
                },
            });
    ...
    private updateMetadataForSerie(serie: Serie): void {
        ...
        const imageUrl =
            this.primaryBackdropUrl() ||
            (serie.poster_path ? getTmdbImageUrl(serie.poster_path, 'w500') : undefined);
    ...
    private updateAllEpisodesInSeason(seasonId: number, markAsWatched: boolean) {
        ...
        if (markAsWatched) {
            const newWatchedEpisodes = seasonEpisodeIds.filter(
                (id) => !currentWatchedEpisodes.includes(id),
            );
            updatedWatchedEpisodes = [...currentWatchedEpisodes, ...newWatchedEpisodes];
        } else {
            updatedWatchedEpisodes = currentWatchedEpisodes.filter(
    ...
    private checkAndUpdateSeasonStatus(season: Season) {
        ...
        const currentWatchedEpisodes = this.watchedEpisodes();
        const seasonEpisodeIds = season.episodes.map((ep) => ep.id);
        const watchedEpisodesInSeason = seasonEpisodeIds.filter((id) =>
            currentWatchedEpisodes.includes(id),
        );
                    ...
                    if (success) {
                        ...
                        if (currentSerie?.user_data) {
                            const updatedWatchedSeasons = [
                                ...(currentSerie.user_data.watched_seasons || []),
                                season.id,
                            ];
                            this.serie.set({
                                ...currentSerie,
