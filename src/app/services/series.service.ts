...
export class SeriesService {
    ...
    getPopularSeries(limit = 6, page = 1): Observable<Serie[]> {
        return this.http
            .get<SearchResponse>(`${this.apiUrl}/series/popular?limit=${limit}&page=${page}`)
            .pipe(
                map((response) => (response.success && response.results ? response.results : [])),
                catchError(() => {
    ...
    getTopRatedSeries(limit = 6, page = 1): Observable<Serie[]> {
        return this.http
            .get<SearchResponse>(`${this.apiUrl}/series/top-rated?limit=${limit}&page=${page}`)
            .pipe(
                map((response) => (response.success && response.results ? response.results : [])),
                catchError(() => {
    ...
    searchSeries(query: string): Observable<Serie[]> {
        ...
        return this.http
            .get<SearchResponse>(`${this.apiUrl}/series/search?q=${encodeURIComponent(query)}`)
            .pipe(
                map((response) => (response.success && response.results ? response.results : [])),
                catchError(() => {
        ...
        return this.http.get<UserSeriesResponse>(`${this.apiUrl}/users/me/series`).pipe(
            map((response) =>
                response.success && response.results
                    ? response.results.map((item) => item.serie)
                    : [],
            ),
    ...
    followSerie(serieId: number): Observable<boolean> {
        return this.http
            .post<{ success: boolean; error?: string }>(
                `${this.apiUrl}/users/me/series/${serieId}/follow`,
                {},
            )
            .pipe(
                map((response) => {
    ...
    unfollowSerie(serieId: number): Observable<boolean> {
        return this.http
            .post<{ success: boolean; error?: string }>(
                `${this.apiUrl}/users/me/series/${serieId}/unfollow`,
                {},
            )
            .pipe(
                map((response) => {
    ...
    markSerieAsWatched(serieId: number): Observable<boolean> {
        return this.http
            .post<{ success: boolean }>(`${this.apiUrl}/users/me/series/${serieId}/watched`, {})
            .pipe(
    ...
    unmarkSerieAsWatched(serieId: number): Observable<boolean> {
        return this.http
            .post<{ success: boolean }>(`${this.apiUrl}/users/me/series/${serieId}/unwatched`, {})
            .pipe(
                map((response) => response.success),
                catchError(() => {
                    this.notificationService.error('notifications.errors.mark_serie');
    ...
    markSeasonAsWatched(serieId: number, seasonId: number): Observable<boolean> {
        return this.http
            .post<{ success: boolean }>(
                `${this.apiUrl}/users/me/series/${serieId}/seasons/${seasonId}/watched`,
                {},
            )
            .pipe(
                map((response) => response.success),
    ...
    unmarkSeasonAsWatched(serieId: number, seasonId: number): Observable<boolean> {
        return this.http
            .post<{ success: boolean }>(
                `${this.apiUrl}/users/me/series/${serieId}/seasons/${seasonId}/unwatched`,
                {},
            )
            .pipe(
                map((response) => response.success),
    ...
    markEpisodeAsWatched(serieId: number, episodeId: number): Observable<boolean> {
        return this.http
            .post<{ success: boolean }>(
                `${this.apiUrl}/users/me/series/${serieId}/episodes/${episodeId}/watched`,
                {},
            )
            .pipe(
                map((response) => response.success),
    ...
    unmarkEpisodeAsWatched(serieId: number, episodeId: number): Observable<boolean> {
        return this.http
            .post<{ success: boolean }>(
                `${this.apiUrl}/users/me/series/${serieId}/episodes/${episodeId}/unwatched`,
                {},
            )
            .pipe(
                map((response) => response.success),
